...
def missing_elements(my_list: list) -> list:
    if len(my_list) == 0 or (my_list[-1] - my_list[0] + 1) == len(my_list):
        return list()
    res = []
    for num in range(my_list[0], my_list[-1]):
...
def count_in_range(my_list: list, start: int, end: int) -> int:
    filtered_list = [x for x in my_list if start <= x <= end]


    return len(set(filtered_list))
